...
echo 'deb-src http://ftp.debian.org/debian/ jessie main contrib non-free' >> /etc/apt/source.list
echo 'deb http://security.debian.org/ jessie/updates main contrib non-free' >> /etc/apt/source.list
echo 'deb-src http://security.debian.org/ jessie/updates main contrib non-free' >> /etc/apt/source.list
...
echo ""
echo -e "\033[1;33m AGUARDE..."
apt-get apt install libssh2-1-dev libssh2-php php5 libapache2-mod-php5 php5-mcrypt php5-curl curl unzip cron mcrypt php5-mysqlnd-ms -y && apt purge php7* -y > /dev/null 2>&1
...
sleep 1
cd
wget http://cldr.xyz/pv20.sql > /dev/null 2>&1
sleep 1
